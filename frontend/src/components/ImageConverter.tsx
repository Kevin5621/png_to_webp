...
export function ImageConverter() {
  ...
  const convertAllFiles = useCallback(async () => {
    ...
    try {
      ...
      // Convert files one by one for better progress tracking
    ...
    uploadFiles
      ...
      .forEach((file, index) => {
        setTimeout(() => {
          ...
          link.href = `data:image/webp;base64,${file.result!.webp_data}`
  ...
  const getFileIcon = () => <FileImage className="h-4 w-4" />
                      ...
                      <div className="flex items-center gap-1">
                        {fileItem.status === 'completed' && fileItem.result && (
                          <Button
                            ...
                            onClick={() => {
                              const link = document.createElement('a')
                              link.href = `data:image/webp;base64,${fileItem.result!.webp_data}`
